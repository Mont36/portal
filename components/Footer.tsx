...
import { navItems, site } from "@/lib/mock-data";
    ...
    <footer className="mt-12 border-t border-white/10 bg-slate-950/70 py-10">
      <div className="container grid gap-8 md:grid-cols-[1fr_1.4fr]">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {navItems.map((item) => (
            <Link key={item.href} href={item.href} className="text-sm font-semibold text-slate-300 hover:text-white">{item.title}</Link>
          ))}
        </div>
      </div>
    </footer>
