...
export function Countdown({ targetDate, compact = false }: CountdownProps) {
  ...
  return (
    <div className={compact ? "grid max-w-[440px] grid-cols-4 gap-2" : "grid grid-cols-2 gap-3 sm:grid-cols-4"} aria-label="Таймер до выхода GTA 6">
      {units.map(([key, label]) => (
        <div key={key} className={compact ? "rounded-xl bg-white/[0.055] p-3 text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.035),0_0_24px_rgba(0,0,0,0.24)] ring-1 ring-white/[0.055]" : "neon-card rounded-2xl p-4 text-center"}>
          ...
          <div className={compact ? "mt-1 text-[0.62rem] font-bold uppercase leading-none tracking-[0.08em] text-slate-300" : "mt-1 text-xs font-bold uppercase tracking-[0.18em] text-cyan-200"}>{label}</div>
        </div>
      ))}
    </div>
  );
