...
const images = [
  "/images/home/map-preview.png",
  "/images/home/characters-preview.png",
  "/images/home/online-preview.png",
  "/images/home/rp-preview.png",
];
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {importantSections.map((item, index) => (
            <article key={item.title} className="group relative min-h-[274px] overflow-hidden rounded-2xl border border-white/[0.08] bg-slate-950 shadow-[0_20px_55px_rgba(0,0,0,0.34)]">
              <Image src={images[index]} alt={item.title} fill sizes="(min-width: 1280px) 25vw, (min-width: 768px) 50vw, 100vw" className="object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-[#03040a] via-[#03040a]/62 to-transparent" />
              <div className="absolute inset-x-0 bottom-0 p-5">
                <h3 className="text-xl font-black uppercase text-white">{item.title}</h3>
                <p className="mt-3 min-h-12 max-w-[230px] text-sm font-semibold leading-6 text-slate-200/90">{item.description}</p>
